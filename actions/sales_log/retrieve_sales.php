<?php
// 1. Fetch the main sales log for today (Your existing query)
...
// 2. Fetch all sales lines for today's sales (New Query)
// We join the products table here so we can grab the actual brand name
$lines_query = "SELECT sl.sale_id, p.brand_name, sl.quantity, sl.price_at_sale 
                ...
                WHERE DATE(log.date_sold) = CURDATE()";
$lines_result = $conn->query($lines_query);

// 3. Group the line items by sale_id for easy access in our panel loop
$sales_lines_grouped = [];
